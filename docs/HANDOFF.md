...
**Branch**: `don`  
**Đang làm**: Iceberg lakehouse integration với MinIO; chuẩn bị rebuild Flink image sau khi chốt Pulsar client 3.0.0 để xác minh job bronze  
...
1. **Iceberg-MinIO Integration Verification**
   - Rebuild Flink image với bộ Pulsar client 3.0.0 và xác minh job bronze hết `NoSuchMethodError`
...
3. **Flink Jobs Development**
   - Bronze job chờ retest sau khi cập nhật Pulsar client 3.0.0; trước đó `RESTARTING` vì thiếu method mới
...
- **Flink SQL Job**: ⚠️ Chưa retest sau khi đóng gói Pulsar client 3.0.0; cần rebuild image rồi chạy lại `bronze_ingest.sql` để xác nhận hết `NoSuchMethodError`
