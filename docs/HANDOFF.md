...
**Mới cập nhật**: Fix Avro schema mismatch bằng cách chuyển sang JSON format; 288 messages đã được ghi thành công vào Bronze layer với 3 Parquet files
...
3. **Performance Optimization**
   - Enable Flink checkpointing (fix Java reflection issue)
...
- **MinIO Setup**: ✅ Warehouse bucket created, 67KiB data stored
- **Pulsar Setup**: ✅ JSON schema registered, 288 messages sent successfully
- **Flink Setup**: ✅ JobManager + TaskManager healthy
- **Flink Bronze Job**: ✅ Completed successfully, consumed all 288 messages
- **Iceberg Bronze Table**: ✅ 3 Parquet files in `warehouse/rva/bronze_raw/data/store_id=store_01/`
...
- **Flink Config**: Checkpointing disabled (tạm thời để tránh Java reflection error)
...
- **Checkpointing**: Disabled temporarily (Java reflection issue with Arrays$ArrayList), không ảnh hưởng demo flow
- **Data Verified**: 288 messages successfully written to 3 Parquet files (20KiB each)
